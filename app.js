...
let componentes = []; // estado del presupuesto
...
// ════════════════════════════════════════════════════════════════════════════
// TABS GENÉRICOS (Componentes y Vida Útil)
// ════════════════════════════════════════════════════════════════════════════
function setupTabs(selectorAttr, panelPrefix) {
  document.querySelectorAll(`[${selectorAttr}]`).forEach(btn => {
    btn.addEventListener("click", () => {
      ...
      // Solo afecta a los paneles relacionados con este prefijo
...
function renderCPUs() {
  ...
  PROCESADORES.forEach(cpu => {
    ...
    if (!activeBrands.includes(marca)) return;
    const color = scoreColor(cpu.score);
    container.innerHTML += `
      <div class="comp-card">
        <h3>${cpu.nombre}</h3>
        <p>🧠 <b>Núcleos:</b> ${cpu.nucleos} núcleos / ${cpu.hilos} hilos</p>
        <p>⚡ <b>TDP:</b> ${cpu.tdp} W</p>
        <p>🔌 <b>Socket:</b> ${cpu.socket}</p>
        <p>💲 <b>Precio:</b> $${cpu.precio} USD</p>
        <div class="card-footer">
          <span class="badge">${cpu.gen}</span>
          <span class="score-tag" style="color:${color};">Score ${cpu.score}/100</span>
        </div>
      </div>`;
  });
    ...
    options: {
      ...
      plugins: {
        ...
        tooltip: {
          ...
          callbacks: {
            label: (ctx) => `${ctx.raw.label}: $${ctx.raw.x} | Score ${ctx.raw.y}`
          }
        }
      },
      scales: {
...
function renderGPUs() {
  ...
  GRAFICAS.forEach(gpu => {
    if (!activeBrands.includes(gpu.marca)) return;
    const color = scoreColor(gpu.score);
    const colorMarca = gpu.marca === "NVIDIA" ? "#76b900" : "#ed1c24";
    container.innerHTML += `
      <div class="comp-card">
        <h3>${gpu.nombre}</h3>
        <p>🖼️ <b>VRAM:</b> ${gpu.vram} GB</p>
        <p>⚡ <b>TDP:</b> ${gpu.tdp} W</p>
        <p>💲 <b>Precio:</b> $${gpu.precio} USD</p>
        <div class="card-footer">
          <span class="badge" style="color:${colorMarca};border-color:${colorMarca};">${gpu.marca}</span>
          <span class="score-tag" style="color:${color};">Score ${gpu.score}/100</span>
        </div>
      </div>`;
  });
...
function renderGPUChart() {
  ...
  new Chart(ctx, {
    ...
    data: {
      ...
      datasets: [{
        label: "Score",
        data: sorted.map(g => g.score),
        backgroundColor: sorted.map(g => g.marca === "NVIDIA" ? "#76b900" : "#ed1c24")
      }]
    },
    options: {
      ...chartDefaultOptions,
...
function renderRAMs() {
  ...
  filtradas.forEach(ram => {
    const color = scoreColor(ram.score);
    const colorTipo = ram.tipo === "DDR5" ? LIME : GREY;
    container.innerHTML += `
      <div class="comp-card">
        <h3>${ram.nombre}</h3>
        <p>📦 <b>Capacidad:</b> ${ram.capacidad} GB</p>
        <p>⚡ <b>Velocidad:</b> ${ram.velocidad} MT/s</p>
        <p>⏱️ <b>Latencia:</b> ${ram.latencia}</p>
        <p>💲 <b>Precio:</b> $${ram.precio} USD</p>
        <div class="card-footer">
          <span class="badge" style="color:${colorTipo};border-color:${colorTipo};">${ram.tipo}</span>
          <span class="score-tag" style="color:${color};">Score ${ram.score}/100</span>
        </div>
      </div>`;
  });
...
function renderRAMChart(filtradas) {
  ...
  // Escala de color según velocidad (similar al gradiente de Plotly)
  ...
  function speedColor(v) {
    ...
    // interpolación simple entre amarillo (#e3b341), lima oscuro y lima
    ...
    options: {
      ...
      plugins: {
        ...
        tooltip: {
          ...chartDefaultOptions.plugins.tooltip,
          callbacks: {
            label: (ctx) => `${ctx.raw.label}: $${ctx.raw.x} | ${ctx.raw.y} GB | ${ctx.raw.vel} MT/s`
          }
        }
      },
      scales: {
...
function normalize(val, campo, items) {
  ...
  if (mx === mn) return 0.5;
  if (campo === "precio" || campo === "tdp") {
    return 1 - (val - mn) / (mx - mn);
  }
  return (val - mn) / (mx - mn);
}
...
function renderVersus() {
  ...
  if (winsA > winsB) {
    winnerEl.innerHTML = `🏆 <b>${sel1}</b> gana en ${winsA} de ${campos.length} especificaciones`;
    winnerEl.style.color = LIME;
  } else if (winsB > winsA) {
    winnerEl.innerHTML = `🏆 <b>${sel2}</b> gana en ${winsB} de ${campos.length} especificaciones`;
    winnerEl.style.color = "#f0883e";
  } else {
    winnerEl.innerHTML = "🤝 Empate técnico — depende de tus prioridades";
    winnerEl.style.color = "#e3b341";
  }
}
      ...
      datasets: [
        {
          ...
          data: rA,
          borderColor: LIME,
          backgroundColor: "rgba(198,241,39,0.2)",
        ...
        },
        {
          ...
          data: rB,
          borderColor: "#f0883e",
          backgroundColor: "rgba(240,136,46,0.2)",
    ...
    options: {
      ...
      scales: {
        r: {
          min: 0,
          max: 1,
          ticks: { color: GREY, backdropColor: "transparent" },
          grid: { color: BORDER },
    ...
    options: {
      ...
      plugins: {
        ...
        tooltip: {
          ...chartDefaultOptions.plugins.tooltip,
          callbacks: {
            label: (ctx) => `${ctx.raw.label}: $${ctx.raw.x.toLocaleString("en-US")} | Score ${ctx.raw.y}`
          }
        }
      },
      scales: {
...
PIEZAS_PRESUPUESTO.forEach(p => {
  budgetTipoSelect.innerHTML += `<option value="${p}">${p}</option>`;
});
...
document.getElementById("budget-add").addEventListener("click", () => {
  ...
  if (nombre === "") {
    warning.textContent = "Escribe el nombre del componente.";
    warning.style.display = "block";
    return;
  }
  if (!precio || precio <= 0) {
    warning.textContent = "Ingresa un precio mayor a 0.";
    warning.style.display = "block";
    return;
  }
  warning.style.display = "none";
...
function renderBudget() {
  ...
  componentes.forEach((c, i) => {
    html += `<tr>
      ...
      <td class="price-cell">$${c.precio.toLocaleString("en-US", { minimumFractionDigits: 2 })}</td>
      <td><button class="btn-del" data-idx="${i}" title="Eliminar">🗑️</button></td>
    </tr>`;
  });
  html += "</tbody>";
  ...
  document.querySelectorAll(".btn-del").forEach(btn => {
    btn.addEventListener("click", () => {
      componentes.splice(parseInt(btn.dataset.idx), 1);
      renderBudget();
    });
  });
  ...
  const avg = total / componentes.length;
  document.getElementById("budget-avg").textContent = `$${avg.toLocaleString("en-US", { minimumFractionDigits: 2 })}`;
...
function renderBudgetChart() {
  ...
  componentes.forEach(c => {
    grouped[c.tipo] = (grouped[c.tipo] || 0) + c.precio;
  });
...
function renderLifeCards() {
  ...
  PROCESADORES.forEach(cpu => {
    ...
    cpuContainer.innerHTML += `
      <div class="life-card">
        <h3>${cpu.nombre}</h3>
        <p class="life-desc">Vida útil estimada: <b>${cpu.vida_anos} años</b></p>
        <div class="life-bar-bg"><div class="life-bar-fill" style="width:${pct}%;"></div></div>
        <div style="margin-top:10px;">${renderUso(cpu.uso)}</div>
      </div>`;
  });
  ...
  GRAFICAS.forEach(gpu => {
    ...
    gpuContainer.innerHTML += `
      <div class="life-card">
        <h3>${gpu.nombre}</h3>
        <p class="life-desc">Vida útil estimada: <b>${gpu.vida_anos} años</b></p>
        <div class="life-bar-bg"><div class="life-bar-fill" style="width:${pct}%;"></div></div>
        <div style="margin-top:10px;">${renderUso(gpu.uso)}</div>
      </div>`;
  });
}
